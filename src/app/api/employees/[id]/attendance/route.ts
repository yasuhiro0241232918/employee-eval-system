...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!month) {
    // 年度集計（有給残数計算用）：4月〜翌3月
    const fiscalYear = Number(req.nextUrl.searchParams.get("fiscalYear") ?? year);
    const start = new Date(Date.UTC(fiscalYear, 3, 1));   // 4月1日
    const end = new Date(Date.UTC(fiscalYear + 1, 3, 1)); // 翌4月1日
